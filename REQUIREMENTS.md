...
77. Implement a useful decision-support upgrade based on the user's delegation, rather than asking the user to interpret the full proposal. Keep Home focused on additional funds, monthly housing cost during the grace period and after month 36, and remaining monthly savings. Connect a selected apartment and unit type to the finance calculation; include acquisition costs, sale costs, reserve cash, credit-loan maturity, and target-specific KB limits. Compare moving against keeping the current home using the same income, actual cash flows, and editable 3/5-year price assumptions without claiming future returns. Add saved candidates, a maximum-three-home comparison, local settings persistence and JSON backup, verified commute inputs, closing cash-flow checks, and monthly CSV export. Replace arbitrary ranking scores with disclosed transaction evidence and affordability sorting, expand candidates through the daily official transaction sync, preserve the existing home-price/reconstruction/map capabilities, and verify desktop/mobile behavior before publishing. Implementation scope and remaining data dependencies are recorded in [the delivered planner guide](docs/DECISION_PLANNER.md).
78. As part of delivery verification for requirement 77, fix the confirmed MOLIT HTTP 429 per-second throttling that left 21 candidate complexes on old data. Pace shared transaction requests across all callers, use bounded retry/backoff for transient failures with Retry-After support, preserve prior values on failure, and expose a selected candidate's stale collection status instead of implying that every successful workflow refreshed every price.
